...
public final class HiveWriteUtils
{
    ...
    {
        try {
            return getRawFileSystem(hdfsEnvironment.getFileSystem(user, path)) instanceof PrestoS3FileSystem;
        }
        catch (IOException e) {
            throw new PrestoException(HIVE_FILESYSTEM_ERROR, "Failed checking path: " + path, e);
        }
    ...
    public static boolean isViewFileSystem(String user, HdfsEnvironment hdfsEnvironment, Path path)
    {
        try {
            ...
            return getRawFileSystem(hdfsEnvironment.getFileSystem(user, path))
